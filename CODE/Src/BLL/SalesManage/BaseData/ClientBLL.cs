...
namespace OA.BLL
{
    ...
    {
        private readonly IClientDAL idal = DALFactory.Helper.GetIClientDAL();

        public List<Client> GetClientsByPage(PageEntity pageEntity, string whereSql = null, string orderBySql = null, bool isForHelp = false)
        {
        ...
        public List<Client> GetClientsByPageForHelp(PageEntity pageEntity, string whereSql = null, string orderBySql = null)
        {
            return idal.GetEntitiesByPageForHelp(pageEntity, whereSql, orderBySql);
        }

        public bool Save(params Client[] entites)
        {
        ...
        public override bool RepeatCheck(Client[] entities)
        {
            ...
            //"客户端"校验
            var sameCode = entities.ToLookup(m => m.ClientCode).Count;
            ...
            var newCodes = (from m in entities
                            where ValidateUtil.isBlank(m.ClientID)
                            select m.ClientCode).ToArray();
