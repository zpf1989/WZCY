function moveup(item, init){
	...
	{ 
		...
		txt.style.top = bottomLinePosition + 500;		
		tempinit = false;
	}

	if(!tempinit && height < boxHeight){
		box.style.height = height + 100;   //100
		box.style.top = bottomLinePosition - height - 100;//100
		txt.style.top = bottomLinePosition - height - 100 - 520;//100
		Stop=setTimeout("moveup(" + item + ",tempinit)",1);
	}else{
