...
public class ClientService : BaseService
{
    ...
    OA.BLL.ClientBLL bll = new OA.BLL.ClientBLL();
    ...
    public void GetListInner(bool isForHelp)
    {
        ...
        string code = Context.Request["ClientCode"];
        if (!ValidateUtil.isBlank(code))
        {
            whereSql += string.Format(" and ClientCode like '%{0}%'", code);
        }
        string name = Context.Request["ClientName"];
        if (!ValidateUtil.isBlank(name))
        {
            whereSql += string.Format(" and ClientName like '%{0}%'", name);
        }
    ...
    public void Save()
    {
        ...
        using (var reader = new System.IO.StreamReader(Context.Request.InputStream))
        {
            ...
            {
                var Clients = data.DeSerializeFromJson<List<Client>>();
