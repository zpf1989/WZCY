<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Upload.ascx.cs" Inherits="OA_UserControl_Upload" %>
<table id='tableA' border="0" cellpadding="0" runat="server">
    <tr>
        <td>
            <div id="MyFile">
                <a href="#?" class="addfile" id="a0">
                    <input type="file"  class="addfile" onchange="addFile()"  name="uploadFile[0].file"
                        value="" /><asp:Button ID="down" runat="server" Width="0" OnClick="Down_OnClick" CausesValidation=false /><input type="hidden" id="selected" runat="server" /><input type="hidden" id="deleted" runat="server" />
                </a>
            </div>
